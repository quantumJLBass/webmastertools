...
class Wsu_WebmasterTools_Controller_Router extends Mage_Core_Controller_Varien_Router_Abstract {
    ...
    public function match(Zend_Controller_Request_Http $request) {
        ...
        foreach ($availableRoutes as $availableRoute) {
            ...
            if ($frontName && $frontName == trim($pathInfo, '/')) {
                ...
                if (!empty($routePath)) {
                    list($routeName, $controllerName, $actionName) = array_merge($routePath, array(
                        'index',
                        'index'
                    ));
    ...
    public function getFrontNameByRoute($routeName) {
        $frontNameInConfig = (string) Mage::getConfig()->getNode(sprintf(self::XML_PATH_FRONT_NAME_FOR_ROUTE, $routeName));
